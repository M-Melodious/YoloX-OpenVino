...
class Network:
    ...
    def load_model(self, model, device, num_requests=0, batch_size=1,
        ...
        assert len(self.net.input_info) == 1, \
        ...
        if num_requests == 0:
            ...
            self.net_plugin = self.plugin.load_network(
                network=self.net, device_name=device, config=tag
                )
        else:
            self.num_requests = num_requests
            self.net_plugin = self.plugin.load_network(
                network=self.net, num_requests=num_requests,
                device_name=device, config=tag
                )
    ...
    def exec_net_async(self, request_id, frame):
        """
    ...
    def exec_net(self, frame):
        ...
        :return: Instance of Executable Network class
        """
        self.infer_request_handle = self.net_plugin.infer(
    ...
    def get_output_async(self, request_id):
        """
        ...
        """
        res = self.net_plugin.requests[request_id].output_blobs[self.out_blob]
    ...
    def get_output(self, output=None):
        ...
        :return: Results
        """
        res = self.infer_request_handle[self.out_blob]
